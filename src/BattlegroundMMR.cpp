...
void BattlegroundMMRMgr::UpdatePlayerRating(Player* player, bool won, const std::vector<Player*>& opponents)
{
    ...
    // Get current rating from external storage
    BattlegroundRatingData currentRating = sGlicko2Storage->GetRating(player->GetGUID());
    ...
    for (Player* opponent : opponents)
    {
        ...
        BattlegroundRatingData oppRating = sGlicko2Storage->GetRating(opponent->GetGUID());
    ...
    // Update external storage (will be saved on logout/periodic save)
    sGlicko2Storage->SetRating(player->GetGUID(), currentRating);

    // Log to history table (async)
    CharacterDatabase.Execute(
        "INSERT INTO character_battleground_rating_history "
        "(guid, rating, rating_deviation, volatility, match_time, match_result) "
        "VALUES ({}, {}, {}, {}, {}, {})",
...
float BattlegroundMMRMgr::GetPlayerMMR(Player* player) const
{
    ...
    // Read from external storage - already cached in memory
    return sGlicko2Storage->GetRating(player->GetGUID()).rating;
}
...
void BattlegroundMMRMgr::InitializePlayerRating(Player* player)
{
    ...
    // Only initialize if not already loaded in storage
    if (!sGlicko2Storage->HasRating(player->GetGUID()))
    {
        ...
        sGlicko2Storage->SetRating(player->GetGUID(), data);
    }
}
